...
/// # Panics
///
/// Panics when logging fails to initialize.
pub async fn init_logging() -> (WorkerGuard, WorkerGuard) {
    ...
    tokio::fs::create_dir_all(&canonical)
        ...
        .unwrap_or_else(|e| {
            panic!(
                "Failed to create canonical directory: {e}. Path: {:?}",
                canonical
            )
        });

    let file_appender =
        tracing_appender::rolling::daily(canonical, "cheesecake.log");
    ...
    let file_log =
        tracing_subscriber::fmt::layer().with_writer(non_blocking_file);
